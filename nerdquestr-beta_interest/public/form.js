...
(() => {
  ...
  function updateRoles(announce = false) {
    ...
    for (const role of roles) {
      byId(`flag-${role.dataset.role}`).value = role.checked ? "1" : "0";
    }
    ...
    if (announce) {
      byId("role-status").textContent =
        `${selected.size} role${selected.size === 1 ? "" : "s"} selected. Art and event details appear for those roles. Optional survey questions also follow your selections.`;
    }
  }
  ...
  function validateField(field) {
    ...
    if (field.required && !field.value.trim()) {
      field.setCustomValidity(
        field.closest("#survey-fields")
          ? "Please complete this field, or skip the optional question using its choice above."
      ...
      if (!Number.isFinite(value) || value < 0) {
        ...
        field.setCustomValidity(
          "Please enter a whole number, including 0, or skip this optional question."
        );
      }
    }
  }
  ...
  byId("use-device-zone").addEventListener("click", () => {
    try {
      ...
      if (
        !Array.from(zoneSelect.options).some(
      ...
      byId("timezone-status").textContent =
        `Selected ${zone.replaceAll("_", " ")}. Change or clear it if that is not where you will take part.`;
    } catch {
      byId("timezone-status").textContent =
  ...
  byId("skip-survey").addEventListener("click", () => {
    ...
    for (const field of byId("survey-fields").querySelectorAll(
    ...
    byId("timezone-status").textContent =
      "Choose where you'll usually take part. We do not request your device location.";
  ...
  function clearCorrectedError() {
    if (
      error.textContent.startsWith("Please check") &&
  ...
  form.addEventListener(
    "invalid",
    (event) => {
      if (surveyPanel.contains(event.target)) {
        surveyPanel.open = true;
      }

      if (
        event.target === roles[0] &&
        !roles.some((role) => role.checked)
      ) {
        byId("roles-error").hidden = false;
      }

      error.textContent =
        "Please check the highlighted fields. Optional survey questions can be skipped.";
  ...
  form.addEventListener("submit", (event) => {
    ...
    for (const field of form.querySelectorAll(
      ...
      if (
        ["text", "email", "url", "textarea"].includes(
          field.type
        )
      ) {
